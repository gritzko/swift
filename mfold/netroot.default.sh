...
$TC filter add dev $EMIF parent ffff: protocol ip prio 1 u32 \
	match u32 0 0 flowid 1:1 action mirred egress redirect dev ifb0 || exit 2

echo adding ifb0 root htb
$TC qdisc add dev ifb0 handle 1: root htb || exit 3

echo adding $EMIF root htb
$TC qdisc add dev $EMIF handle 1: root htb || exit 4
